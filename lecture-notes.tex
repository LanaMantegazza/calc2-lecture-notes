...
\begin{definition}\label{pathrerf}
    Let $\fpath{r}{n}$ be a function. Then $\mathbf{r}$ is a \textbf{path} on $\rbbn$
\end{definition}
\begin{definition}
    Let $C \subseteq \rbbn$ be a subset of points in $\rbbn$. If there exists some \hyperlink{\hypergetref{pathrerf}}{path} $\func{\mathbf{r}}{\rbb}{C\subseteq\rbbn}$ such that $\mathbf{r}$ is continuous, then C is a \textbf{curve}.

continuous subsets $C \subseteq \rbbn$
\end{definition}
% \begin{example}
